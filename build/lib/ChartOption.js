...
class ChartOption {
    ...
    themeType;
    chart;
    isTouch = typeof window !== 'undefined' ? 'ontouchstart' in window.document.documentElement : false;
    compact;
    lastFormattedTime;
    option;
    debug = false;
    ...
    */
    ...
    convertData(data, chartIndex, yAxisIndex) {
    ...
    static getCommonAxis(commonYAxis, chartIndex) {
        return commonYAxis === '' || commonYAxis === undefined
            ? chartIndex
            : parseInt(commonYAxis, 10) || 0;
    }
            ...
            if (oneLine.chartType === 'bar') {
                const cfg = {
                    ...
                    label: {
                        ...
                        position: this.config.barLabels === 'topover'
                            ? 'top'
    ...
    getYAxis(series) {
        return this.config.l.map((oneLine, chartIndex) => {
            ...
            if (yAxis[chartIndex]) {
                const diff = yAxis[chartIndex].max - yAxis[chartIndex].min;
                if (Number.isNaN(yMin)) {
                    // auto calculate
                    yMin = yAxis[chartIndex].min - diff * 0.1; // min - 10%
                    if (diff > 25000) {
                ...
                if (Number.isNaN(yMax)) {
                    // auto calculate
                    yMax = yAxis[chartIndex].max + diff * 0.1; // max + 10%
                    if (diff > 25000) {
                ...
                for (let i = 0; i < 2; i++) {
                    ...
                    series.markLine.data.push({
                        ...
                        lineStyle: {
                            ...
                            shadowBlur: parseFloat(oneMark.os)
                                ? parseFloat(oneMark.os) + 1
                                : 0,
                            shadowOffsetY: parseFloat(oneMark.os)
                                ? parseFloat(oneMark.os) + 1
                                : 0,
                        ...
                        label: {
                            ...
                            distance: [
                                oneMark.textPosition === 'r' || oneMark.textPosition === 'l'
                                    ...
                                    : parseFloat(oneMark.textOffset) || 0,
                                0,
                            ],
                            fontStyle: 'normal',
    ...
    yFormatter(val, line, withUnit, interpolated, forAxis) {
        ...
        if (this.config.l[line].states) {
            ...
            const state = this.config.l[line].states[strNumValue];
        ...
        if (interpolated) {
            simpleValue = Math.round(simpleValue * 10000) / 10000;
        }
        if (this.config.useComma) {
            simpleValue = parseFloat(simpleValue) || 0;
    ...
    getInterpolatedValue(seriesIndex, ts, type, hoverNoNulls) {
